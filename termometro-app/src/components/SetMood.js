...
function SetMood({ token, history }) {
    ...
    return (
        <section className="sliderContainer">
            {!tooManyMoods && userInfo && <div className='sliderContainer__titleContainer'>
                <h1 className='sliderContainer__title'>Qué tal está tu autoestima hoy?</h1>
            </div>}
            {!tooManyMoods && userInfo &&<div className='sliderContainer__thermoContainer'>
                ...
                {!tooManyMoods && userInfo && <input type="range" min="0" max="10" className="sliderContainer__thermoContainer--slider" value={value} onChange={handleOnChange}></input>}
            {!tooManyMoods && userInfo && <button onClick={handleSetMood} className='sliderContainer__button'>Enviar</button>}


            {tooManyMoods && userInfo && <div className='tooManyMoodsContainer'>
